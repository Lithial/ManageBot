...
- **`max_workers` is per-run config (`runs.max_workers` column).** Resolved at submit time via the default chain `SubmitRunRequest.MaxWorkers` → daemon `--max-workers` flag (default source, default 4) and persisted on the run; `api.NewServer` takes the daemon default. `driveWorkers` reads `run.MaxWorkers` (falling back to `o.cfg.MaxWorkers` for pre-migration rows, then the built-in default). `GET /runs/{id}` exposes the effective value; `wrap run --max-workers N` overrides per run.
