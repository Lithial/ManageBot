...
Phase 9 (real MCP + e2e smoke) is on branch `phase-9-mcp` (built atop phase 8); it's split into 3 PRs — see `docs/superpowers/plans/2026-06-05-phase-9-mcp.md`. **9a (done):** `cmd/wrap-mcp` is the MCP stdio bridge `claude` spawns as a worker's `wrap` tool provider (built on the official `github.com/modelcontextprotocol/go-sdk`); it exposes the five `wrap.*` tools and relays each call to the daemon over the Unix socket, scoped by `--worker <id>`. The daemon grew worker-facing endpoints (`GET /workers/{id}/task|siblings`, `POST /workers/{id}/progress|done|blocked` → `worker_report_*` events) + `store.GetWorker` + client worker methods. **9b (done):** the planner/worker/merger all report over MCP. The planner and merger now get `workers` rows (reserved task ids `__planner__`/`__merger__`, excluded from the merger's surviving-branch set) so they can be MCP-scoped; the planner reports via `wrap.report_plan` → `worker_report_plan`. The orchestrator determines outcomes from `worker_report_*` events (dual-path: `workerOutcome`/`plannerPlan` fall back to stdout NDJSON when no MCP report exists, which is what keeps the `fake-claude`-NDJSON unit tests green). `wrapd` builds per-worker `claude -p --mcp-config <inline JSON → wrap-mcp --worker id> --append-system-prompt <prompt-dir>/<role>.md` and sets `WRAP_WORKER_ID`/`WRAP_MCP_SOCKET` (which also drive `fake-claude`'s MCP mode — so integration tests run the real event-driven path). Prompts live in `prompts/{planner,worker,merger}.md`; flags `--wrap-mcp-cmd`/`--prompt-dir`. **9c (done):** `test/integration/e2e_test.go` (`//go:build integration && e2e`) runs one real `claude` through the whole pipeline (planner→worker→merger over real MCP via the `wrap-mcp` bridge + `prompts/`) against a trivial fixture, asserting the run reaches `done` and the merged branch contains the worker's committed artifact (not its exact LLM text — content fidelity is deliberately untested). Skipped unless `claude` is on PATH; never in `make test`; run via `make test-e2e`. **Verified PASS against real `claude` 2.1** (~106s). This completes Phase 9 — the spec is fully implemented.

Real-`claude` integration notes (learned wiring this up against `claude` 2.1):
- **`wrapd` spawns `claude -p --permission-mode auto --setting-sources project --strict-mcp-config --mcp-config <inline JSON> --append-system-prompt <role>.md`.** `--mcp-config` accepts inline JSON. `--setting-sources project` is load-bearing: it isolates the spawned worker from the operator's *global* `~/.claude` settings (hooks, output styles) — an "explanatory" output style there pushes claude toward prose instead of tool calls. `--strict-mcp-config` loads only the `wrap` server.
- **The tool-call imperative must be in the user turn (stdin), not just `--append-system-prompt`.** The orchestrator appends `plannerDirective`/`workerDirective`/`mergerDirective` to each subprocess's stdin; the appended system prompt alone is unreliable (claude answers in prose). `fake-claude` ignores stdin, so these are no-ops in the shim tests.
- MCP tools are namespaced `mcp__wrap__report_plan` etc.; the role prompts refer to them as `report_plan`/`report_done` and claude maps them.
...
make wrapd         # daemon
make wrap-mcp      # MCP stdio bridge claude spawns as its `wrap` tool provider
...
make test-e2e                             # opt-in real-claude smoke (skips if claude absent; spends API)
...
- **`Reconcile` runs once at `wrapd` startup, before the tick loop.** It fails runs caught in a single-subprocess phase (`planning`/`merging`) — resuming one in-flight planner/merger is unsafe — but **resumes** runs caught mid-`working` from partial progress: the run is left at `working` (not failed), orphaned `running` workers are failed as retryable, and the tick loop's `resumeWorkers` re-derives completed tasks from `worker_done` events (`completedTasks` → `scheduleFrom` seed) so finished work is not repeated and only unfinished/orphaned tasks are re-dispatched. Gate/pending runs are left for the loop. `working` is in the tick loop (`driveByPhase(PhaseWorking, resumeWorkers)`) but only fires after a crash — a normal tick races `plan_gate → working → merging` synchronously so no run rests at `working` between ticks. Reasons live in events (`daemon_recovered{reason:resume}` for the working path vs `{reason:daemon_restart}` for failed `planning`/`merging`; `worker_failed{reason:daemon_restart}`), not new columns.
